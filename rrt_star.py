...
def dynamic_in_out():
      ...
      if (x-340)**2 + (y-y1)**2 <= (5)**2:
          dynamic_points.append((x,y))
      if (x- x1)**2 + (y - 100)**2 <= (5)**2:
          dynamic_points.append((x,y))
#plotting
...
def next_node(node):
    ...
    while True:
        ...
        if obs_in_out((new_x,new_y)) ==True and new_x>=0 and new_x<=640 and new_y>=0 and new_y<=480:    
...
def cost_update(node):
   
    cost={}
    new_node,success = next_node(node)
    if success == True:
...
def obs_in_out(point):
    
    if point in static_points:
        return False
    else:
        return True
        ...
        while True:
            ...
            for itr in range(50):
                ...
                if obs_in_out(curr_vert) == True:
                    next_cost = cost_update(curr_vert)
                    ...
                    for key,val in next_cost.items(): #key is node and val is cost from parent to that node
                        ...
                        if len(neighbor_nodes) > 0:
                           ...
                           node_index = temp.index(min_cost)
                            ...
                            node_index = 0
                        cost_now = Eucledian(next_vert,neighbor_nodes[node_index])    
...
goal_found, backtracking_dict= rrt_star(start, goal)
